...
def login():
    ...
    yhteys = get_connection()
    cursor = yhteys.cursor()
    cursor.execute("select id from player where screen_name = %s", (username,))
    result = cursor.fetchone()
...
def signup():
    # Haetaan tiedot, jotka frontend lähettää
    data = request.get_json() or {}
    username = data.get("name")
    password = data.get("password")
    ...
    try:
        query_db("INSERT INTO users (username, password, signup_timestamp, age) VALUES (%s, %s, NOW(), 0)",
                 (username, hashed), commit=True)

        query_db("INSERT INTO player (screen_name, location, budget) VALUES (%s, 'EFHK', 20000)",
                 (username,), commit=True)

        return jsonify({"message": "Käyttäjä luotu onnistuneesti! Voit nyt kirjautua sisään."}), 201

    except Exception as e:

        print("[Signup Database Error]:", e)

        return jsonify({"error": "Virhe käyttäjän luonnissa. Tarkista tietokantayhteysasetukset (DB_CONFIG)."}), 500
...
def select_flight():
    ...
    if price is not None:
        new_budget = session['budget'] - price
        if new_budget < 0:
            minigame = "lose"
        else:
